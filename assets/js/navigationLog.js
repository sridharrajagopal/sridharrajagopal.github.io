...
const debouncedSaveAll = debounce(function () {
...
function loadAllFromLocalStorage() {
  ...
  if (!raw) {
    ...
    try {
      ComputeResults(document.forms[0]);
    } catch (e) {}
    return;
  }
  ...
  if (elementsList && elementsList.length) {
    ...
    const curElems = Array.from(document.querySelectorAll("input, textarea, select")).filter(function (el) {
      return !!el.name;
    });
...
function createCellWithInput(row, inName, colspan = 1, rowspan = 1, readonly = false, disabled = false) {
  var cell = row.insertCell();
...
function ComputeColRem(totalFuel, cells) {
  var total = 0.0;
  if (totalFuel.value == 0) total = 0.0;
  else total = parseFloat(totalFuel.value);
...
function ComputeThetaW(cell1, cell2, cell3) {
  ...
  if (wind >= Math.round(cell2.value)) cell3.value = Math.abs(Math.round(cell2.value) - wind) % 360;
  else cell3.value = 360 - (Math.abs(Math.round(cell2.value) - wind) % 360);
}
...
document.addEventListener("input", function (e) {
  if (e.target && (e.target.tagName === "INPUT" || e.target.tagName === "TEXTAREA" || e.target.tagName === "SELECT")) debouncedSaveAll();
});
